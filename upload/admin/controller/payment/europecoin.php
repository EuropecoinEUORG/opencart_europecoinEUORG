...
class ControllerPaymentEuropeCoin extends Controller {
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/'.$this->payment_module_name)) {
        	...
        	if (!$this->request->post['europecoin_rpc_username']) {
			$this->error['username'] = $this->language->get('error_username');
		}

	        if (!$this->request->post['europecoin_rpc_address']) {
        		$this->error['address'] = $this->language->get('error_address');
	        }

        	if (!$this->request->post['europecoin_rpc_password']) {
		        $this->error['password'] = $this->language->get('error_password');
	        }

        	if (!$this->request->post['europecoin_rpc_port']) {
		        $this->error['port'] = $this->language->get('error_port');
	        }

        	if (!$this->request->post['europecoin_prefix']) {
		        $this->error['prefix'] = $this->language->get('error_prefix');
	        }

        	if (!$this->request->post['europecoin_countdown_timer']) {
	...
	public function index() {
		...
		$this->data['text_2'] = $this->language->get('text_2');
        	...
        	$this->data['entry_show_erc'] = $this->language->get('entry_show_erc');
	        $this->data['entry_erc_decimal'] = $this->language->get('entry_erc_decimal');
		...
		//end language code
        	...
        	$this->data['breadcrumbs'][] = array(
	            ...
	            'href'      => $this->url->link('payment/europecoin', 'token=' . $this->session->data['token'], 'SSL'),
	            'separator' => ' :: '
        	);
		...
		$rows = $result->rows;
		$max = $result->num_rows;
		$europecoin_total_in_db = 0;
		$europecoin_address_in_db = 0;

		for($i = 0;$i < $max;$i++) {
			if($rows[$i]["Field"] == "europecoin_total") {
				$europecoin_total_in_db = 1;
			}
			if($rows[$i]["Field"] == "europecoin_address") {
				$europecoin_address_in_db = 1;
			}
		}

		if(!$europecoin_total_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD europecoin_total DOUBLE AFTER currency_value;");
		}

		if(!$europecoin_address_in_db) {
			$this->db->query("ALTER TABLE ". DB_PREFIX ."order ADD europecoin_address VARCHAR(34) AFTER europecoin_total;");
		}
		...
		if (isset($this->request->post[$this->payment_module_name.'_prefix'])) {
	        ...
	        if (isset($this->request->post[$this->payment_module_name.'_show_erc'])) {
			$this->data[$this->payment_module_name.'_show_erc'] = $this->request->post[$this->payment_module_name.'_show_erc'];
		} else {
			$this->data[$this->payment_module_name.'_show_erc'] = $this->config->get($this->payment_module_name.'_show_erc');
		}

	        if (isset($this->request->post[$this->payment_module_name.'_erc_decimal'])) {
			$this->data[$this->payment_module_name.'_erc_decimal'] = $this->request->post[$this->payment_module_name.'_erc_decimal'];
		} else {
			$this->data[$this->payment_module_name.'_erc_decimal'] = $this->config->get($this->payment_module_name.'_erc_decimal');
		}
